...
public abstract class PersistenceSessionDelegateImpl implements PersistenceSession{
	...
	private final static Logger logger=Logger.getLogger(PersistenceSessionDelegateImpl.class.getSimpleName());
	...
	public PersistenceSessionDelegateImpl(final EntityManager _session){
		...
		logger.log(Level.FINEST,SimpleFormat.supplier("persistence-session(delegated)[{}]::create::{}",getKind(),this));
	}
	@SuppressWarnings({"OverridableMethodCallInConstructor", "LeakingThisInConstructor"})
